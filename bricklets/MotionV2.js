...
module.exports = function(RED) {
	...
	function tinkerForgeMovementV2(n) {
        ...
        function(connectReason) {
            node.md = new Tinkerforge.BrickletMotionDetectorV2(node.sensor, node.ipcon);
            node.md.on(Tinkerforge.BrickletMotionDetectorV2.CALLBACK_MOTION_DETECTED,detected);

            // Register detection cycle ended callback
            node.md.on(Tinkerforge.BrickletMotionDetectorV2.CALLBACK_DETECTION_CYCLE_ENDED,ended);
        });
    ...
    RED.nodes.registerType('TinkerForge MotionV2', tinkerForgeMovementV2);
}
