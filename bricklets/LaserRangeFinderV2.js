...
module.exports = function (RED) {
  function tinkerForgeLaserRangeFinderV2(n) {
    ...
    node.ipcon.on(
      ...
      function (connectReason) {
        node.t = new Tinkerforge.BrickletLaserRangeFinderV2(
          node.sensor,
          node.ipcon
        );
        ...
        node.interval = setInterval(function () {
          if (node.t) {
            node.t.getDistance(
              function (distance) {
                node.send({
                  topic: node.topic || "LaserRangeFinderV2",
                  payload: distance,
                });
              },
              function (err) {
  ...
  RED.nodes.registerType(
    "TinkerForge LaserRangeFinderV2",
    tinkerForgeLaserRangeFinderV2
  );
};
